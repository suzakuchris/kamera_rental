...
<script>
    ...
    function search_process(){
        ...
        $.ajax({
            type    : 'POST',
            url     : '{{route("master.item_condition.search")}}',
        ...
        $.each(data, function(x,y){
            ...
            rows += `
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            <button class="btn btn-outline-primary d-flex align-items-center" onclick="edit_data(`+y.condition_id+`)"><i class="bi bi-pencil me-2"></i>Edit</button>
                            <button class="btn btn-outline-danger d-flex align-items-center" onclick="delete_data(`+y.condition_id+`)"><i class="bi bi-trash me-2"></i>Delete</button>
                        </div>
                    </td>
                </tr>
            `;
    ...
    function edit_data(id){
        ...
        $.ajax({
            type    : 'POST',
            url     : '{{route("master.item_condition.view")}}',
            headers : { "X-CSRF-TOKEN": "{{ csrf_token() }}" },
            dataType: 'JSON',
            data    : {
                'condition_id':id,
    ...
    function fill_form(data){
        var form = $("#form_modal form");
        form.trigger('reset');
        form.find("[name='condition_id']").val(data.condition_id);
        form.find("[name='condition_name']").val(data.condition_name)
    }
    ...
    function save_form(form){
        ...
        $.ajax({
            type        : 'POST',
            url         : '{{route("master.item_condition.upsert")}}',
            headers     : { "X-CSRF-TOKEN": "{{ csrf_token() }}" },
            dataType    : 'JSON',
            data        : dtForm,
            success     : function(msg) {
    ...
    function delete_data(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type    : 'POST',
                    url     : '{{route("master.item_condition.delete")}}',
                    headers : { "X-CSRF-TOKEN": "{{ csrf_token() }}" },
                    dataType: 'JSON',
                    data    : {
                        'condtion_id':id,
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="formModalLabel">Data Kondisi</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form onsubmit="save_form(this);">
                    <input type="text" class="d-none" name="condition_id">
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group">
                                <label>Nama Kondisi</label>
                                <input type="text" class="form-control" name="condition_name" required placeholder="Masukan nama kondisi">
                            </div>
                        </div>
                    </div>
